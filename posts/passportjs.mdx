...
2. Get authentication from Google
   ...
   get ClientID and ClientSecret which we save in our .env file
   to prevent leakage.Then set url and redirect url in Google console, so we can get
   the information we want from Google and redirect for us to manipulate
   the data and save in database.

3. Deployment into Heroku
